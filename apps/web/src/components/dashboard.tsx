...
const stats = [
  { label: 'Alumnos activos', value: '486', change: '+14,2%', tone: 'text-emerald-600', helper: 'vs. curso anterior' },
  { label: 'Colegios', value: '12', change: '+2', tone: 'text-emerald-600', helper: 'este curso' },
  { label: 'Grupos activos', value: '38', change: '+8,6%', tone: 'text-emerald-600', helper: 'vs. curso anterior' },
  { label: 'Profesores', value: '21', change: '+9,0%', tone: 'text-emerald-600', helper: 'vs. curso anterior' },
]
...
const categories = ['Robótica y tecnología', 'Idiomas', 'Deporte', 'Arte y creatividad', 'Apoyo escolar']
...
{index === 0 ? 'Recibos cobrados' : index === 1 ? 'Pendientes de cobro' : index === 2 ? 'Por remesa SEPA' : 'Recibos impagados'}
</p>
                  </div>
                ))}
              </div>
...
Favoritos
            ...
            <div className="mt-4 grid grid-cols-2 gap-2">
              {[['Nuevo alumno', UserPlus], ['Colegios', School], ['Grupos', Trophy], ['Agenda', CalendarDays]].map(([label, Icon]) => (
                <button
key={label as string}
...
No se encontraron resultados.
                  ...
                  <CommandGroup heading="Navegación">
                    {nav.map(([label, Icon]) => <CommandItem
key={label}
value={label}
onSelect={() => goTo(label)}
                    >
...
function PanelHeader({ title, icon: Icon, action }: { title: string; icon: typeof Users; action?: string }) {
  return (
...
{title}
      ...
      {action && <button className="ml-auto text-xs font-medium text-primary hover:underline">
{action}
</button>}
...
Este módulo se construirá sobre la nueva base de colegios, profesores, alumnos y actividades.
</p>
</div>
    </Card>
  )
